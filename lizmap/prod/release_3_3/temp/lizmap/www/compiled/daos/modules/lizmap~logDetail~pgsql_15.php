...
if (jApp::config()->compilation['checkCacheFiletime']&&(
 ...
 filemtime('C:\webserver\lizmap\prod\release_3_3\lizmap/modules/lizmap/daos/logDetail.dao.xml') > 1616160726)){ return false;
}
else {
 require_once ( JELIX_LIB_PATH .'dao/jDaoRecordBase.class.php');
 require_once ( JELIX_LIB_PATH .'dao/jDaoFactoryBase.class.php');

class cDaoRecord_lizmap_Jx_logDetail_Jx_pgsql extends jDaoRecordBase {
 public $id;
 public $key;
 public $timestamp;
 public $user;
 ...
 public $ip;
   ...
   public function getProperties() { return cDao_lizmap_Jx_logDetail_Jx_pgsql::$_properties; }
   public function getPrimaryKeyNames() { return cDao_lizmap_Jx_logDetail_Jx_pgsql::$_pkFields; }
}

class cDao_lizmap_Jx_logDetail_Jx_pgsql extends jDaoFactoryBase {
  ...
  array (
   ...
   protected $_selectClause='SELECT "log_detail"."id", "log_detail"."log_key" as "key", "log_detail"."log_timestamp" as "timestamp", "log_detail"."log_user" as "user", "log_detail"."log_content" as "content", "log_detail"."log_repository" as "repository", "log_detail"."log_project" as "project", "log_detail"."log_ip" as "ip"';
   protected $_fromClause;
   protected $_whereClause='';
   protected $_DaoRecordClassName='cDaoRecord_lizmap_Jx_logDetail_Jx_pgsql';
...
'.($record->key === null ? 'NULL' : $this->_conn->quote2($record->key,false)).', current_timestamp, '.($record->user === null ? 'NULL' : $this->_conn->quote2($record->user,false)).', '.($record->content === null ? 'NULL' : $this->_conn->quote2($record->content,false)).', '.($record->repository === null ? 'NULL' : $this->_conn->quote2($record->repository,false)).', '.($record->project === null ? 'NULL' : $this->_conn->quote2($record->project,false)).', '.($record->ip === null ? 'NULL' : $this->_conn->quote2($record->ip,false)).'
   ...
   if(!$result)
       return false;
   if($record->id < 1 ) 
          $record->id= $this->_conn->lastInsertId('log_detail_id_seq');
