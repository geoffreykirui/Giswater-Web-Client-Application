...
if (jApp::config()->compilation['checkCacheFiletime']&&(
 ...
 filemtime('C:\webserver\lizmap\prod\release_3_3\lizmap/modules/lizmap/daos/geobookmark.dao.xml') > 1616160726)){ return false;
}
else {
 require_once ( JELIX_LIB_PATH .'dao/jDaoRecordBase.class.php');
 require_once ( JELIX_LIB_PATH .'dao/jDaoFactoryBase.class.php');

class cDaoRecord_lizmap_Jx_geobookmark_Jx_pgsql extends jDaoRecordBase {
 public $id;
 public $login;
 public $name;
 public $map;
 public $params;
   ...
   public function getProperties() { return cDao_lizmap_Jx_geobookmark_Jx_pgsql::$_properties; }
   public function getPrimaryKeyNames() { return cDao_lizmap_Jx_geobookmark_Jx_pgsql::$_pkFields; }
}

class cDao_lizmap_Jx_geobookmark_Jx_pgsql extends jDaoFactoryBase {
  ...
  array (
   ...
   protected $_selectClause='SELECT "geobookmark"."id", "geobookmark"."usr_login" as "login", "geobookmark"."bname" as "name", "geobookmark"."bmap" as "map", "geobookmark"."bparams" as "params"';
   protected $_fromClause;
   protected $_whereClause='';
   protected $_DaoRecordClassName='cDaoRecord_lizmap_Jx_geobookmark_Jx_pgsql';
...
'.($record->login === null ? 'NULL' : $this->_conn->quote2($record->login,false)).', '.($record->name === null ? 'NULL' : $this->_conn->quote2($record->name,false)).', '.($record->map === null ? 'NULL' : $this->_conn->quote2($record->map,false)).', '.($record->params === null ? 'NULL' : $this->_conn->quote2($record->params,false)).'
   ...
   if(!$result)
       return false;
   if($record->id < 1 ) 
          $record->id= $this->_conn->lastInsertId('geobookmark_id_seq');
    return $result;
}
