...
if (jApp::config()->compilation['checkCacheFiletime']&&(
 ...
 filemtime('C:\webserver\lizmap\prod\release_3_3\lizmap/modules/lizmap/daos/logCounter.dao.xml') > 1616160726)){ return false;
}
else {
 require_once ( JELIX_LIB_PATH .'dao/jDaoRecordBase.class.php');
 require_once ( JELIX_LIB_PATH .'dao/jDaoFactoryBase.class.php');

class cDaoRecord_lizmap_Jx_logCounter_Jx_pgsql extends jDaoRecordBase {
 public $id;
 public $key;
 public $counter;
 public $repository;
 public $project;
   ...
   public function getProperties() { return cDao_lizmap_Jx_logCounter_Jx_pgsql::$_properties; }
   public function getPrimaryKeyNames() { return cDao_lizmap_Jx_logCounter_Jx_pgsql::$_pkFields; }
}

class cDao_lizmap_Jx_logCounter_Jx_pgsql extends jDaoFactoryBase {
  ...
  array (
   ...
   protected $_selectClause='SELECT "log_counter"."id", "log_counter"."key", "log_counter"."counter", "log_counter"."repository", "log_counter"."project"';
   protected $_fromClause;
   protected $_whereClause='';
   protected $_DaoRecordClassName='cDaoRecord_lizmap_Jx_logCounter_Jx_pgsql';
...
'.($record->key === null ? 'NULL' : $this->_conn->quote2($record->key,false)).', '.($record->counter === null ? 'NULL' : intval($record->counter)).', '.($record->repository === null ? 'NULL' : $this->_conn->quote2($record->repository,false)).', '.($record->project === null ? 'NULL' : $this->_conn->quote2($record->project,false)).'
   ...
   if(!$result)
       return false;
   if($record->id < 1 ) 
          $record->id= $this->_conn->lastInsertId('log_counter_id_seq');
    return $result;
}
